...
- **願望清單**：收集想去的地點、投票否決、批次排進行程，人在附近（500m）會推播提醒
...
- `WORKERS_URL` — Cloudflare Workers 邀請短碼 backend（可選，沒設會 fallback 長 base64 邀請碼；部署方式見 `workers/invite-shortcode/SETUP.md`）
...
每個群組的 `BroTrip-Data` Sheet 有 **10 個分頁**：
