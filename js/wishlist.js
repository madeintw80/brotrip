...
const Wishlist = {
  ...
  // v3.9.1: 只有 emoji（地圖大頭針 label 用，不含中文）
  TYPE_EMOJI: {
    restaurant: '🍜',
    attraction: '🏛️',
    cafe: '☕',
  ...
  // v3.9.1: 各分類代表色（地圖大頭針 + 篩選 chip 共用，白字可讀）
  TYPE_COLOR: {
    restaurant: '#ef4444', // 紅 — 餐廳
    attraction: '#3b82f6', // 藍 — 景點
    cafe: '#d97706',       // 琥珀 — 咖啡
    experience: '#9333ea', // 紫 — 體驗
    shopping: '#ec4899',   // 粉 — 購物
    other: '#6b7280',      // 灰 — 其他
  },
  ...
  async forceRestore(id) {
    const existing = this.allList.find(w => w.id === id);
    // 先取投票名單再清票 — _updateRow 會就地把 rejected_votes 改成 '[]'，
    // 之後才讀 getVoters 永遠拿到空陣列，「否決被覆寫」通知從來沒發出去過
    const voters = existing ? this.getVoters(existing) : [];
