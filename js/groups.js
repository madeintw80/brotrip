...
// 11 個分頁的 schema（Sheet 建立時用，建群組時直接從這裡推導，加分頁不會脫鉤）
// v3.9.14 新增 Reactions（日記表情反應）
...
const GROUP_SCHEMA = {
  ...
  // v3.9.14 新增：日記表情反應（👍❤️😂😮😢🔥），一人一篇可按多種、再按取消
  Reactions: ['id', 'diary_id', 'author', 'emoji', 'created_at'],
...
const Groups = {
  ...
  //   6. addSheetTabs(...)                    → 加 Trips 以外的所有 tab（依 GROUP_SCHEMA）
  //   7. setSheetHeaders × N                  → 寫所有 tab 的 headers
  //   8. 加進 list，切換 active
  ...
  async create(name, onProgress) {
    ...
    try {
      ...
      // Step 6: 加 Trips 以外的所有 tab（直接從 GROUP_SCHEMA 推導，未來加分頁不會脫鉤）
      progress(6, 9, '建立資料表分頁...');
      const otherTabs = Object.keys(GROUP_SCHEMA).filter((t) => t !== 'Trips');
      const otherTabIds = await API.addSheetTabs(sheetId, otherTabs);
      const sheetTabIds = { Trips: tripsTabId, ...otherTabIds };

      // Step 7: 寫所有 tab 的 headers
      progress(7, 9, '寫入欄位定義...');
