...
const Diaries = {
  ...
  async create(data, onProgress) {
    ...
    const newDiary = {
      ...
      pinned: '',
      // 新群組 sheet header 叫 drive_folder_url、legacy TGL 叫 url — 記憶體物件兩個鍵都放，
      // 後續 render / 重寫 row 不管讀哪個鍵都拿得到
      url: driveFolderUrl,
  ...
  async update(id, data) {
    ...
    const newRow = [
      ...
      existing.created_at,
      existing.pinned || '',
      // 兩鍵相容（schema 叫 drive_folder_url、legacy 叫 url）— 只讀 .url 會把新群組的連結洗成空白
      existing.drive_folder_url || existing.url || '',
      mentionsJson,
    ];
