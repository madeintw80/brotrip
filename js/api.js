...
const API = {
  ...
  // ⚠️ 一定要用 =s（最長邊）不要用 =w（寬度）：
  //   =w 會「直接縮原始像素、跳過 EXIF 方向、又把 EXIF 標籤拔掉」→ 手機直拍/側拍
  //      (EXIF Orientation≠1) 的照片會躺著顯示，瀏覽器也無從得知要轉。
  //   =s 會「先套用 EXIF 方向再縮」→ 所有方向都正確（含現有已上傳的照片，免重傳）。
  driveImageUrl(fileId, size = 400) {
