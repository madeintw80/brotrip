...
const Trips = {
  ...
  async delete(tripId) {
    ...
    // Wishlist 也是 trip 關聯資料 — 漏刪會留 orphan rows，重建同名 trip_id 舊願望會復活
    const wishIds = (typeof Wishlist !== 'undefined')
      ? Wishlist.allList.filter(w => w.trip_id === tripId).map(w => w.id)
      : [];
    ...
    if (wishIds.length > 0) await API.batchDeleteRows('Wishlist', wishIds);
