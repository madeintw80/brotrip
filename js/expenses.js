...
const Expenses = {
  ...
  // v3.9.16：移除原本「settled=TRUE 就跳過這筆 expense」的邏輯。
  //   那個旗標是已下架的「一鍵全清」按鈕（v3.9.6 移除）留下的殘留設計。
  //   風險：當一筆 expense 被標 settled=TRUE、又同時有對應的 confirmed settlement（還款收據）時，
  //   會變成「帳單被整筆跳過、還款卻照樣計入」→ 產生反向假債務（明明還了錢，系統倒過來說對方欠你）。
  //   修法：回歸單一模型 = 所有 expense 都計入，債務一律靠 confirmed settlements 抵銷。
  //   settled 欄位（M 欄）保留不動（避免改到列結構），但不再被讀取，等同退役。
  settle() {
    const balances = {};
  ...
  async update(id, data) {
    ...
    // v3.9.15: 非 trip 成員（圍觀者）不能改 — 補上 v3.8.0 角色制度漏掉的 gate
    //   （當時只擋了新增 FAB，編輯/刪除忘了擋，造成圍觀者能改別人 trip 的帳）
    if (typeof App !== 'undefined' && !App.isCurrentTripMember()) {
      throw new Error('你不是這個 trip 的成員，只能檢視支出');
    }
  ...
  async delete(id) {
    const existing = this.list.find(e => e.id === id);
    if (!existing) throw new Error('找不到該支出');
    // v3.9.15: 非 trip 成員（圍觀者）不能刪 — 同 update，補上 v3.8.0 漏掉的 gate
    if (typeof App !== 'undefined' && !App.isCurrentTripMember()) {
      throw new Error('你不是這個 trip 的成員，只能檢視支出');
    }
