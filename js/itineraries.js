...
const Itineraries = {
  ...
  // ===== v3.9.2: 批次排行程（一次多個 wish）=====

  // 用「多個」wish 建一個新 itinerary（waypoints = 全部選到的 wish）
  async createFromWishes(wishes, { name, travel_mode } = {}) {
    if (!Trips.current) throw new Error('沒有當前 trip');
    // 只收有有效座標的 wish（itinerary 需要 lat/lng）
    const valid = (wishes || []).filter(w => {
      const lat = parseFloat(w.lat), lng = parseFloat(w.lng);
      return !isNaN(lat) && !isNaN(lng);
    });
    if (valid.length === 0) throw new Error('沒有有座標的願望可加');

    const itinName = (name && name.trim()) ||
      (valid.length === 1 ? `${valid[0].name} 行程` : `${valid[0].name} 等 ${valid.length} 個地點`);
    const waypoints = valid.map(w => ({
      name: w.name,
      address: w.address || '',
      lat: parseFloat(w.lat),
      lng: parseFloat(w.lng),
      place_id: w.place_id || '',
      wishlist_id: w.id,   // 雙向 link
    }));

    const id = API.newId();
    const createdAt = new Date().toISOString();
    // 維持 7 column 跟原 create() 一致（wishlist_id 帶在 waypoint JSON 內）
    const row = [
      id, Trips.current.trip_id, itinName,
      JSON.stringify(waypoints),
      travel_mode || 'DRIVING',
  ...
  // 把「多個」wish 一次 append 到既有 itinerary（已在行程內的自動跳過）
  async addWaypointsFromWishes(itineraryId, wishes) {
    const itin = this.allList.find(i => i.id === itineraryId);
    if (!itin) throw new Error('找不到該行程');
    ...
    (wishes || []).forEach(w => {
      ...
      if (waypoints.find(x => x.wishlist_id === w.id)) return; // 已在行程內 → 跳過
      waypoints.push({
        name: w.name, address: w.address || '',
        lat, lng, place_id: w.place_id || '', wishlist_id: w.id,
      });
      added++;
    });
    if (added === 0) throw new Error('選的願望都已經在這個行程裡了');
